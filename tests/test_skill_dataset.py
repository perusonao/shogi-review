...
class CohortAndDashboardTests(unittest.TestCase):
    ...
    def test_repository_snapshot_excluding_growth_baseline_games(self):
        rows, _ = collect(ROOT)
        ...
        self.assertEqual(report["pilot_cohort"]["player_games"], 30)
        self.assertEqual(report["pilot_cohort"]["unique_users"], 16)
        self.assertEqual(report["pilot_cohort"]["stages"]["stage_1"]["total_shortage"], 60)

    def test_latest_main_counts_are_default_growth_baseline(self):
        rows, _ = collect(ROOT)
        report = dataset_dashboard(build_dataset(rows, default_route="existing-kif"))
        self.assertEqual(report["all_data"]["player_games"], len(rows))
        self.assertEqual(report["pilot_cohort"]["player_games"], 32)
        self.assertEqual(report["pilot_cohort"]["unique_users"], 17)
        self.assertEqual(report["pilot_cohort"]["by_rank"]["2級"]["player_games"], 3)
        self.assertEqual(report["pilot_cohort"]["by_rank"]["1級"]["player_games"], 9)
        self.assertEqual(report["pilot_cohort"]["by_rank"]["初段"]["player_games"], 20)
        self.assertEqual(report["pilot_cohort"]["stages"]["stage_1"]["total_shortage"], 58)
        self.assertEqual(
